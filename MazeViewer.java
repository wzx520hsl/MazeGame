...
public class MazeViewer {
   ...
   */
   private static MazeFrame readMazeFile(String fileName) throws IOException {
	  ...
	  Scanner in=new Scanner(new File(fileName));//The scanner that scan the file
	  Scanner readLine=new Scanner(in.nextLine());//The scanner that scan each line
	  int numRows=readLine.nextInt();
	  int numCols=readLine.nextInt();
	  boolean[][] maze=new boolean[numRows][numCols];
	  //Scan all the maze
	  for(int j=0;j<numRows;j++){
		  String line=in.nextLine();
		  for(int i=0;i<numCols;i++){
			  char checkWall=line.charAt(i);
			  if(checkWall==FREE_CHAR){
				  maze[j][i]=true;
	  ...
	  //Scan the entry and exit position
	  readLine=new Scanner(in.nextLine());
      int yStart=readLine.nextInt();
      int xStart=readLine.nextInt();
      readLine=new Scanner(in.nextLine());
      int yEnd=readLine.nextInt();
      int xEnd=readLine.nextInt();
      
      return new MazeFrame(maze, new MazeCoord(yStart, xStart), new MazeCoord(yEnd, xEnd));
